...
- **`push`** — refuses if `verify` fails. Materializes each slice as a `drip/<branch>/sliceN` branch and opens a PR via `gh` for each one that doesn't already have a correspondence entry (re-running updates the existing branch/PR instead of duplicating it — see `docs/adr/0006-slice-correspondence-key.md`). `--dry-run` previews without touching GitHub; otherwise `--yes` is required, since this and `stack link` are the only commands with real external side effects.
  ...
  - `--reviewable-stack` requires every base to be something a reviewer can open: the repository's default branch, a branch itself under review as a PR, or a prerequisite projection's own PR branch. Four things it refuses, and where:
    - **A generated integration base.** Under `flat-first` a projection with two prerequisites gets a minted `drip/<branch>/<id>-base` branch that has **no PR**: GitHub merges the child fine, but reviewers can't walk the prerequisites as a stack and a workflow filtered on `pull_request.branches: [main]` never runs on it. Without the flag every such PR is still flagged — in its result line, its note and a summary — because that situation should never be discovered after the fact. Refusal propagates to anything that depended on the refused projection, since its PR would target a branch that was never pushed.
    - **A `--base` that isn't a branch.** A commit sha, a tag, `origin/main` — `git rev-parse` resolves all of them and `gh pr create` accepts none. Refused before anything is materialized, naming which of them it was.
    - **A base branch nothing reviews.** A branch published by hand to stand in for a prerequisite is the same hidden base as one drip mints. Refused unless it is the repository's default branch or has an open PR of its own; the remedy named is never "let drip publish a branch", it's to declare that work as a projection and depend on it.
    - **A prerequisite with no PR branch.** The base a dependent targets has to be a branch this run leaves standing with a PR on it, not one it is assumed to.

    Every base is printed with what makes it reviewable — `base: main (default branch)`, `base: drip/f/shared (shared-contract, #12)`, or in a dry-run `(shared-contract, PR opens in this run)` — so the plan answers "which branch owns my prerequisite" before the push. If GitHub can't be read, a real push refuses and a dry-run says it couldn't confirm. See `docs/adr/0032-reviewable-bases.md`.
