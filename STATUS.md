...
M3, M4, and M5's deterministic half are built and verified live. M5's AI conveniences were deliberately re-scoped away from a bundled provider integration toward a `--json` machine-readable interface for external tools (`docs/adr/0009`) — that's built and verified too. Everything from the original plan is now either built or explicitly re-scoped by user decision. The one thing left: the M0 kill-gate, still explicitly deferred by the user in favor of continuing to build, not overlooked — it's the last ungated assumption everything else rests on.
