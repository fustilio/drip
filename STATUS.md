...
| M5 — DAG parallelism + AI | Parallelism half built, verified live. AI half not started. | `src/verify.ts`, `src/signature.ts` (+`store.ts` `build_cache` table). Per-slice build results are cached by content hash and reused for an unbroken unchanged *prefix* of the stack — not full DAG independence, see `docs/adr/0008-build-cache-scope.md` for why (the stack is a single linear commit chain, so a later slice's build genuinely depends on everything before it). Slices that do need rebuilding run concurrently via `Promise.all` since each already gets its own git worktree. The three AI conveniences (`--ai`-gated slice naming, fuzzy comment relocation, boundary suggestions) are unbuilt — needs a provider/API-key decision first. |
...
M3 and M4 are now built and verified live (M4 in its deliberately conservative, tier-1-only cut — see `docs/adr/0007`). M5's deterministic half (build-cache prefix-skip + parallel builds) is built and verified live too. What's left of the original plan: M5's AI conveniences (needs a provider/API-key decision — not started), and the M0 kill-gate, which remains explicitly deferred by the user in favor of continuing to build, not overlooked.
