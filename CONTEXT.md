...
**Tree-hash invariant**:
The core correctness check: `apply(slices in topological order) == tree(mega branch)`, verified by comparing git tree hashes. In M0, this is the only verification performed — no per-slice standalone build check (that's M1 scope).
